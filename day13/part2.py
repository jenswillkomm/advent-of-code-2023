...
def find_row_reflection(puzzle):
    for row in range(puzzle.shape[0] - 1):
        # print('Line at %s. Pairs: %s' % (str(row), str([(i, j) for i, j in zip(range(row, -1, -1), range(row + 1, puzzle.shape[0]))])))
        if np.count_nonzero(~np.array([(puzzle[i, :] == puzzle[j, :]) for i, j in zip(range(row, -1, -1), range(row + 1, puzzle.shape[0]))])) == 1:
            return row + 1
    return -1
...
def find_col_reflection(puzzle):
    for col in range(puzzle.shape[1] - 1):
        if np.count_nonzero(~np.array([(puzzle[:, i] == puzzle[:, j]) for i, j in zip(range(col, -1, -1), range(col + 1, puzzle.shape[1]))])) == 1:
            return col + 1
    return -1
